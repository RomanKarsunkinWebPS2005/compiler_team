...
| Ассоциативность | Операторы |
|------------------|-----------|
| **Правоассоциативные** | `=`, `+=`, `-=`, `*=`, `/=`, `%=`, и другие составные операторы присваивания<br>`?:` (тернарный)<br>`throw`, `typeid`, `sizeof`, `decltype`, `co_await`<br>`dynamic_cast`, `static_cast`, `const_cast`, `reinterpret_cast` |
| **Левоассоциативные** | `+`, `-`, `*`, `/`, `%`, `<<`, `>>`, `&`, `\|`, `^`, `&&`, `\|\|` |
...
- **Python**:
  - `<<` и `>>` — **бесконечные** сдвиги (для чисел произвольной длины)
  - Отрицательные числа сдвигаются как есть (арифметический сдвиг не применяется)

- **C++**:
  - Поведение зависит от типа: для **знаковых** типов — **неопределено** при отрицательных значениях или сдвиге больше разрядности
  - Для **беззнаковых** — определено как арифметический сдвиг
